...
class Tool_Agents:
    def to_link_and_peek(link: str, state: State):
        ...
        for tag in soup.find_all(functional_tags):
            ...
            tag_string = str(tag)
            # why this?
            lxml_element = parser.xpath(
                f"//*[text()='{tag.get_text(strip=True)}' or @*='{tag.attrs.get(list(tag.attrs.keys())[0], '')}']")
    ...
    # 有意义的元素：["form", "input", "button", "select", "option", "textarea", "table", "tr", "td", "th", "a"]
    def find_target_element(target_type: str, target_des: str, state: State):
        ...
        '''
        # 假设我们访问website之后马上来这里吧
        website_functional_element = state['websites_list'][-1][3]

        # 设计prompt
        '''
        我们要在以下string中找到符合target type和target description的元素
        你能在找到之后返回xpath吗（需要上下文也许）
